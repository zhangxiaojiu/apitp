...
class UserModel extends Model
{
    ...
    public function doCreateTeam($id,$isMoney = false)
   {
        ...
        $user = Db::name('user')->find($id);
          if($user['pid']>0){
            ...
            if($puser['user_type'] == 3){
                $ptjcoin = $gjuserSettings['ptj'];
            }else{
                $ptjcoin = $ptuserSettings['ptj'];
            }
            $data_f_p = [
                'uid' => $user['pid'],
                'coin' => $ptjcoin,
            ...
            if($puser['pid']>0){
              ...
              if($ppuser['user_type'] == 3){
                  $pptjcoin = $gjuserSettings['pptj'];
              }else{
                  $pptjcoin = $ptuserSettings['pptj'];
              }
              $data_f_pp = [
                  'uid' => $puser['pid'],
                  'coin' => $pptjcoin,
                  'type' => 'team',
                  'detail' => '下下级id'.$id.'团建奖励',
                  'status' => 1,
              ]; 
